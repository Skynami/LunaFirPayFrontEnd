...
import { Card, Table, Form, Input, Button, Tag, Alert, Modal, Radio, Checkbox, Switch, message, Tooltip, Typography } from 'antd'
...
function RAM() {
  ...
  const allPermissions = [
    { value: 'admin', label: '管理员', desc: '拥有除RAM管理外的所有权限' },
    { value: 'order', label: '流水管理', desc: '查看订单流水、补发通知' },
    { value: 'finance', label: '提现管理', desc: '提现申请/取消、查看余额' },
    { value: 'settings', label: '设置管理', desc: '修改结算信息、Telegram绑定' }
  ]
  ...
  const fetchMembers = async () => {
    setLoading(true)
    try {
      const res = await api.get('/api/merchant/ram/list')
      if (res.data.code === 0) {
        setMembers(res.data.data || [])
      }
    } catch (error) {
      console.error('获取RAM成员失败:', error)
    } finally {
      setLoading(false)
    }
  }
  ...
  const addMember = async () => {
    ...
    try {
      ...
      const res = await api.post('/api/merchant/ram/add', {
  ...
  const saveMember = async () => {
    ...
    try {
      ...
      const res = await api.post('/api/merchant/ram/update', {
        id: editFormData.id,
  ...
  const deleteMember = (row) => {
    Modal.confirm({
      ...
      onOk: async () => {
        try {
          const res = await api.post('/api/merchant/ram/delete', { id: row.id })
          if (res.data.code === 0) {
    ...
    },
    {
      ...
      render: (permissions) => {
        ...
        return perms.length > 0 ? (
          perms.map(perm => (
            <Tag key={perm} color={perm === 'admin' ? 'red' : 'blue'}>
    ...
    { title: '创建时间', dataIndex: 'created_at', width: 170, render: (v) => formatTime(v) },
    {
      ...
      render: (_, row) => (
        <>
          ...
          <Button type="link" danger size="small" onClick={() => deleteMember(row)}>删除</Button>
        </>
      )
    }
  ]
  ...
  return (
    <div>
      <h2 className="page-title">RAM子账户管理</h2>
      ...
      <Alert
        ...
        description={
          <ul style={{ margin: '8px 0', paddingLeft: 20 }}>
            ...
            <li>账号格式为13位数字，由系统自动生成，无法自定义</li>
            <li>密码为18位随机字符，创建后请立即保存</li>
            <li><strong>管理员</strong>：除了RAM管理外的所有权限</li>
            <li><strong>流水管理</strong>：查看流水、导出流水、给用户退款</li>
          </ul>
        }
        type="info"
        showIcon
